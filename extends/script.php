<?php
class hubby_mexin_theme_handler
{
	public $news;	
	public $file;
	private $data;
	...
	public function __construct($data)
	{
		...
		$this->data['file']->css_push('bootstrap');
		$this->data['file']->css_push('responsive');
		$this->data['file']->css_push('style');
		$this->data['file']->css_push('touchTouch');
		...
		$this->data['file']->js_push('jquery.easing.1.3');
		$this->data['file']->js_push('jquery.cookie');
		$this->data['file']->js_push('touchTouch.jquery');
		$this->data['file']->js_push('bootstrap');		
	...
	*/
	...
	{
		$this->data['file']->css_push('camera');
		$this->data['file']->js_push('camera.min');
	...
	*/
	...
	public function defineForm($a)
	{
		...
		{
			$text	=	'<label class="control-label">'.$a['text'].'</label>';
		}
		else
		{
			$text	=	'';
		...
		if(array_key_exists('type',$a))
		{
			if($a['type']	==	'textarea')
			{
				$balise	=	'<div class="controls"><textarea '.$name.'>'.$value.'</textarea></div>';
			}
			else
			{
				$balise	=	'<div class="controls"><input class="input-medium" '.$name.' '.$subtype.' '.$value.'/></div>';
			}
		}
		else
		{
		...
		$this->currentForm[]	= '<div class="control-group input-medium">'.$text.$balise.'</div>';
	}
